import {
  COMPONENT_QUERY_GRID, COMPONENT_QUERY_ITEM, COMPONENT_QUERY_ITEM_BUTTON, COMPONENT_QUERY_ITEM_SELECT,
  definitionQueryGrid, definitionQueryItem, definitionQueryItemButton, definitionQueryItemSelect,
...
export function generateSchema(definitions?: Record<string, ISchema>, fields?: Pick<IQueryFieldItem, 'field' | 'operatorOptions'>[]): ISchema {
  const mergeDefinitions: Record<string, ISchema> = {
    ...
    [COMPONENT_QUERY_ITEM_BUTTON]: definitionQueryItemButton,
  ...
  const properties = fields?.reduce<Record<string, ISchema>>((previousValue, currentValue, currentIndex) => {
    ...
    const payload = {
      [field]: {
        ...
        properties: {
          field: {
            '$ref': `#/definitions/${COMPONENT_QUERY_ITEM_BUTTON}`,
            'x-component-props.field': field,
          },
          operator: {
